...
var RegressionNetParams = Params{
	...
	// Private chain: Genesis + Chronicle active from height 1 (the first post-genesis block) for priv-chain bsv
	// parity. The gobdk script verifier hard-rejects a 0/negative activation height (SetGenesisActivationHeight
	// panics), so 1 is the lowest valid value. The genesis block (height 0) carries only the coinbase, so every
	// funding/BOLT tx (height >= 1) is Genesis-active -> the pre-Genesis 520-byte push limit never applies. No
	// mining to the default 10000 activation height is required.
